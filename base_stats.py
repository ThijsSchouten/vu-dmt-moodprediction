...
def plot_sensors_per_hour(df, output_path, size=(9, 3)):
    ...
    df_sensors_hourly = df_sensors_only[['hour', 'id', 'value']].groupby([
                                                                         'hour'])
    df_sensors_hourly = df_sensors_hourly.aggregate(
        {'id': 'count', 'value': 'mean'})
...
def plot_sensors_per_date(df, output_path, size=(9, 3)):
    ...
    df_sensors_only = df[df['variable'] != 'mood']
    df_sensors_daily = df_sensors_only[['date', 'id', 'value']].groupby([
                                                                        'date'])
    df_sensors_daily = df_sensors_daily.aggregate(
        {'id': 'count', 'value': 'mean'})
